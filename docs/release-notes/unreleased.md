...
- fix(mcp): `npm i -g mindwtr-mcp` no longer needs a C++ compiler on Windows — better-sqlite3 is now ^12 with prebuilt binaries for current Node (mindwtr-mcp 1.1.0, Node 20+).
