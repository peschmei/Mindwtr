...
- feat(desktop): update checks are now channel-aware for every install channel — Scoop and Chocolatey installs are detected (no more GitHub download prompts fighting your package manager), winget/Scoop/Chocolatey/Homebrew/AUR users get the update reminder only when their own channel has published the new version and are routed to that channel's update path, and Flatpak/Snap/App Store builds stay quiet because those update automatically. (#829)
