...
This is the third release candidate for Mindwtr 1.1.0. Use it for tester channels only. It supersedes `v1.1.0-rc.2`, which was tagged but not published after release validation exposed an MCP package version-stamping problem.
...
- Smoke desktop undo after completing a Today task, short-list pull-to-refresh on mobile, and MCP install on Windows with Node 20+.
...
- Updated `mindwtr-mcp` 1.1.1 so global npm installs can use current prebuilt `better-sqlite3` binaries on Node 20+.
- Kept MCP package versions independent from app release version stamping so app RCs cannot overwrite npm package fixes.

## Full RC3 Change List (since `v1.1.0-rc.1`)
...
- fix(mcp): `npm i -g mindwtr-mcp` no longer needs a C++ compiler on Windows with Node 20+ (mindwtr-mcp 1.1.1).
