...
- Updated `mindwtr-mcp` so global npm installs can use current prebuilt `better-sqlite3` binaries on Node 20+.
...
- fix(mcp): `npm i -g mindwtr-mcp` no longer needs a C++ compiler on Windows with Node 20+.
